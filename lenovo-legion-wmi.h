...
#define __LL_ATTR_RO_AS(_name, _show)                                          \
  { .attr = {.name = __stringify(_name), .mode = 0444}, .show = _show, }
...
/* Attribute property show/store */
#define __LL_TUNABLE_RW(_prop, _attrname, _dev_id, _feat_id)                   \
  static ssize_t _attrname##_##_prop##_store(struct kobject *kobj,             \
                                             struct kobj_attribute *attr,      \
                                             const char *buf, size_t count) {  \
    return attr_compat_data_store(kobj, attr, buf, count,                      \
                                  &om_wmi.ll_tunables->_attrname, _dev_id,     \
                                  _feat_id);                                   \
  }                                                                            \
  static ssize_t _attrname##_##_prop##_show(struct kobject *kobj,              \ 
                                            struct kobj_attribute *attr,       \
                                            char *buf) {                       \
    return attr_compat_data_show(kobj, attr, buf, _dev_id, _feat_id);          \
  }                                                                            \
  static struct kobj_attribute attr_##_attrname##_##_prop =                    \
      __LL_ATTR_RW(_attrname, _prop);

/* Attribute property show only */
#define __LL_TUNABLE_RO(_prop, _attrname, _dev_id, _feat_id)                   \
  static ssize_t _attrname##_##_prop##_show(                                   \
      struct kobject *kobj, struct kobj_attribute *attr, char *buf) {          \
    return attr_compat_data_show(kobj, attr, buf, _dev_id, _feat_id);          \
  }                                                                            \
...
#define ATTR_GROUP_LL_TUNABLE(_attrname, _fsname, _dev_id, _feat_id, _incstep, \
                              _dispname)                                       \
  __LL_TUNABLE_RW(current_value, _attrname, _dev_id, _feat_id);                \
  __LL_TUNABLE_RO(default_value, _attrname, _dev_id, _feat_id);                \
  __ATTR_SHOW_FMT(display_name, _attrname, "%s\n", _dispname);                 \
  __LL_TUNABLE_RO(max_value, _attrname, _dev_id, _feat_id);                    \
  __LL_TUNABLE_RO(min_value, _attrname, _dev_id, _feat_id);                    \
